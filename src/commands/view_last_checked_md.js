...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('view_last_checked_md')
        .setDescription('View last checked times for Maryland stores')
    ...
    async execute(interaction) {
        ...
        if (storeOption) {
            // Show specific store
            await handleLastCheckedSpecificStore(interaction, 'md', storeOption);
        } else {
            // Show select menu to choose between all or specific store
            await handleLastCheckedMenu(interaction, 'md');
        }
    }
};
