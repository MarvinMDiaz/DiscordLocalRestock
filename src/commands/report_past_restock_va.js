...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('report_past_restock_va')
        .setDescription('Report a past restock at a specific store (Virginia) - logs only, no alert')
    ...
    async execute(interaction) {
        try {
            // Gate channel access
            const gate = gateChannel('report_past_restock_va', interaction.channelId);
            if (!gate.allowed) {
            ...
            // Check cooldowns (but past restocks shouldn't trigger store cooldowns)
            ...
            // Create restock report (marked as past_restock)
            ...
            // Add cooldown for user (but past restocks don't trigger store cooldowns)
            const cooldown = {
                user_id: userId,
                store: store,
            ...
            console.error('❌ Error in report_past_restock_va command:', error);
            if (!interaction.replied && !interaction.deferred) {
                await interaction.reply({
                    content: '❌ There was an error submitting your past restock report. Please try again.',
                    ephemeral: true
                });
            }
        }
