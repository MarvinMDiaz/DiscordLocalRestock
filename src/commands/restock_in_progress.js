...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('restock_in_progress_va')
        .setDescription('Report a restock currently in progress - Virginia (with photo support)')
        ...
        .addStringOption(option =>
            option.setName('line_estimate')
                .setDescription('Current line/crowd size at the store')
                .setRequired(true)
                .addChoices(
                    { name: '🟢 1–10 people', value: 'low' },
                    { name: '🟡 10–20 people', value: 'moderate' },
                    { name: '🔴 20+ people', value: 'high' },
                    { name: '⚪ Unknown / I Don’t Know', value: 'unknown' }
                )
        ),
    ...
    async autocomplete(interaction) {
        ...
        if (focusedOption.name === 'location') {
            ...
            let stores = [];
            if (storeType === 'target') {
                stores = config.stores.target.va || [];
            } else if (storeType === 'bestbuy') {
                stores = config.stores.bestbuy.va || [];
            } else if (storeType === 'walmart') {
                stores = config.stores.walmart.va || [];
            } else if (storeType === 'barnesandnoble') {
                stores = config.stores.barnesandnoble.va || [];
            }
    ...
    async execute(interaction) {
        try {
            // Gate channel access
            const gate = gateChannel('restock_in_progress_va', interaction.channelId);
            if (!gate.allowed) {
            ...
            const lineEstimate = crowdTracker.normalizeEstimate(interaction.options.getString('line_estimate'));
            ...
            // Create restock report with "in progress" flag
            ...
            const approvalEmbed = new EmbedBuilder()
                .setColor(0xFF6B35) // Orange-red for in-progress
            ...
            const { createApprovalProofThread, getAdminMentions, buildApprovalButtonRows, sendReporterReportSubmittedDm } = require('../utils/approvalManager');
            ...
            try {
                if (approvalChannelId && approvalChannelId.trim() !== '') {
                    ...
                    if (approvalChannel) {
                        ...
                        console.log('⚠️ Approval channel not found in cache.');
                    }
                } else {
                    console.log('⚠️ Approval channel ID missing or invalid.');
                }
            } catch (sendErr) {
                console.error('⚠️ Could not send to approval channel:', sendErr.message);
                // Do not fail the interaction for this
            }
            ...
            // Reply to user
            await interaction.editReply({
                content: `✅ **In-Progress Restock Report Submitted!**\n\n🏪 **Store**: ${store}\n\nYour report has been submitted and is awaiting approval. This will be logged for historical tracking.`,
                ephemeral: true
            });

        } catch (error) {
            console.error('❌ Error in restock_in_progress command:', error);
            if (!interaction.replied && !interaction.deferred) {
                await interaction.reply({
                    content: '❌ There was an error submitting your in-progress alert. Please try again.',
                    ephemeral: true
                });
            }
        }
