...
function lineEstimateRow(sessionId) {
    const select = new StringSelectMenuBuilder()
        .setCustomId(`line_estimate_${sessionId}`)
        .setPlaceholder('How many people are currently in line?')
        .addOptions(crowdTracker.estimateSelectOptions());
    return new ActionRowBuilder().addComponents(select);
}

function proofFinalEmbedColor(reportKind) {
    if (reportKind === 'in_progress') return 0xff6b35;
    if (reportKind === 'past') return 0xffa500;
    return 0x0099ff;
}

/**
 * Last UI step before submit: optional proof photo (private mod thread) — no logic change on confirm handlers.
 */
function buildProofPhotoFinalPayload(sessionId, region, reportKind) {
    const confirmCustomId =
        reportKind === 'in_progress'
            ? `confirm_in_progress_${region}_${sessionId}`
            : reportKind === 'past'
              ? `confirm_past_${region}_${sessionId}`
              : `confirm_upcoming_${region}_${sessionId}`;

    const photoUseValue =
        reportKind === 'in_progress'
            ? '• Helps moderators verify the report.\n• If the report is **approved**, a proof photo **may be shared** in the public alert discussion thread.'
            : '• Helps moderators verify the report.\n• Used during review only (no public alert for this report type).';

    const embed = new EmbedBuilder()
        .setColor(proofFinalEmbedColor(reportKind))
        .setTitle('Almost done')
        .setDescription('_Use **Submit Report** below to finish, or **Cancel** to stop._')
        .addFields(
            {
                name: 'After you tap Submit Report',
                value:
                    '• You get a **private thread** on the moderator approval message.\n' +
                    '• Upload a **clear photo** there if you want (always optional).',
                inline: false
            },
            {
                name: 'How proof photos are used',
                value: photoUseValue,
                inline: false
            },
            {
                name: 'Tip',
                value:
                    'A clear photo often speeds up review when moderators are busy.\n\n' +
                    'Nothing goes public until a moderator approves.',
                inline: false
            }
        )
        .setFooter({ text: 'Proof is optional — encouraged when you can attach a clear image.' });
...
function buildInProgressConfirmPayload(cachedData, sessionId) {
    const estimate = crowdTracker.formatEstimate(cachedData.line_estimate);
    const fields = [
        { name: '🏪 Store', value: cachedData.store, inline: false },
        { name: '📅 Type', value: 'Restock In Progress', inline: true },
        { name: '📍 Region', value: cachedData.region.toUpperCase(), inline: true },
        { name: '👥 Current Line', value: estimate, inline: true }
    ];

    if (cachedData.newlook_alerts === true) {
        fields.push({ name: '🆕 Flow', value: 'New Look (consolidated)', inline: true });
    }
    ...
    const confirmEmbed = new EmbedBuilder()
        ...
        .addFields(fields)
        .setFooter({ text: 'Tap Continue for final confirmation, or Cancel to stop.' });

    const confirmRow = new ActionRowBuilder()
        .addComponents(
            new ButtonBuilder()
                .setCustomId(`report_review_next_inprog_${cachedData.region}_${sessionId}`)
                .setLabel('Continue')
    ...
    return { embeds: [confirmEmbed], components: [confirmRow], ephemeral: true };
}

function lineEstimatePromptPayload(cachedData, sessionId) {
    const embed = new EmbedBuilder()
        .setColor(0x3498DB)
        .setTitle('👥 Current Line Size')
        .setDescription('Select the current line/crowd size for this restock report.')
        .addFields(
            { name: '🏪 Store', value: cachedData.store, inline: false },
            { name: '📍 Region', value: cachedData.region.toUpperCase(), inline: true }
        )
        .setFooter({ text: 'This starts the live crowd tracker if the alert is approved.' });

    return { embeds: [embed], components: [lineEstimateRow(sessionId)], ephemeral: true };
}
...
async function handleLocationSelect(interaction, region) {
    try {
        ...
        await interaction.update(lineEstimatePromptPayload(modalDataCache.get(sessionId), sessionId));
...
async function handlePastRestockDateSelect(interaction, region) {
    try {
        ...
        const confirmRow = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
                    .setCustomId(`report_review_next_past_${region}_${sessionId}`)
                    .setLabel('Continue')
                    .setStyle(ButtonStyle.Primary),
                new ButtonBuilder()
                    .setCustomId(`cancel_report_${sessionId}`)
...
async function processPastRestockSubmission(interaction, region, store, storeType, restockDate, dateInput, sessionId) {
    try {
        ...
        const { createApprovalProofThread, getAdminMentions, buildApprovalButtonRows, sendReporterReportSubmittedDm } = require('./approvalManager');
        const approvalRows = buildApprovalButtonRows(restockId);
        ...
        try {
            if (approvalChannelId && approvalChannelId.trim() !== '') {
                ...
                if (approvalChannel) {
                    ...
                    await createApprovalProofThread(approvalMessage, restockReport);
                    postedToApproval = true;
        ...
        if (!postedToApproval) {
            await sendReporterReportSubmittedDm(interaction.client, {
...
async function handleUpcomingRestockNoteSubmit(interaction, region) {
    try {
        ...
        const confirmRow = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
                    .setCustomId(`report_review_next_upcoming_${region}_${sessionId}`)
                    .setLabel('Continue')
                    .setStyle(ButtonStyle.Primary),
                new ButtonBuilder()
                    .setCustomId(`cancel_report_${sessionId}`)
...
async function handleCustomStoreNameInProgress(interaction, region) {
    try {
        ...
        await interaction.reply(lineEstimatePromptPayload(cachedData, sessionId));
...
function getStoreType(storeName) {
    ...
    if (name.startsWith('walmart')) return 'walmart';
...
function getStoreTypeDisplay(storeType) {
    const types = {
        ...
        'walmart': { name: 'Walmart', emoji: '🛒', color: 0x0071CE },
...
async function handleLookupButtonClick(interaction, region) {
    try {
        ...
        const embeds = [];
        const storeTypeOrder = ['target', 'bestbuy', 'walmart', 'barnesandnoble', 'other'];
        ...
        for (const storeType of storeTypeOrder) {
            ...
            for (const storeData of stores) {
                ...
                try {
                    ...
                    } else if (storeType === 'walmart' && displayName.toLowerCase().startsWith('walmart - ')) {
...
/**
 * Handle required current-line estimate before final in-progress confirmation
 */
async function handleLineEstimateSelect(interaction) {
    try {
        const sessionId = interaction.customId.replace('line_estimate_', '');
        const cachedData = modalDataCache.get(sessionId);
        if (!cachedData || cachedData.userId !== interaction.user.id) {
            return await interaction.update({
        ...
        cachedData.line_estimate = crowdTracker.normalizeEstimate(interaction.values[0]);
        modalDataCache.set(sessionId, cachedData);

        await interaction.update(buildInProgressConfirmPayload(cachedData, sessionId));
    } catch (error) {
        console.error('❌ Error handling line estimate select:', error);
        if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({
                content: '❌ There was an error saving the line estimate. Please try again.',
                ephemeral: true
            });
        }
    }
...
/**
 * Review step → final confirmation (proof photo notice) before existing submit handlers run.
 */
async function handleReportReviewNext(interaction, flow, region, sessionId) {
    try {
        const cachedData = modalDataCache.get(sessionId);
        if (!cachedData || cachedData.userId !== interaction.user.id) {
            return await interaction.update({
        ...
        if (flow === 'inprog' && !cachedData.line_estimate) {
            return await interaction.update({
                content: '❌ Please select the current line size before continuing.',
                embeds: [],
                components: [lineEstimateRow(sessionId)],
                ephemeral: true
            });
        }

        if (cachedData.region && cachedData.region !== region) {
            return await interaction.update({
                content: '❌ Session mismatch. Please start over.',
                embeds: [],
                components: [],
                ephemeral: true
            });
        ...
        const reportKind = flow === 'inprog' ? 'in_progress' : flow;
        await interaction.update(buildProofPhotoFinalPayload(sessionId, region, reportKind));
    } catch (error) {
        console.error('❌ Error advancing to final report confirmation:', error);
        if (!interaction.replied && !interaction.deferred) {
            await interaction.update({
                content: '❌ There was an error. Please try again.',
                embeds: [],
...
async function handleConfirmInProgress(interaction, region) {
    try {
        ...
        const lineEstimate = crowdTracker.normalizeEstimate(cachedData.line_estimate);

        if (!cachedData.line_estimate) {
            return await interaction.update({
                content: '❌ Please select the current line size before submitting.',
                embeds: [],
                components: [lineEstimateRow(sessionId)],
                ephemeral: true
            });
        ...
        // Acknowledge before slow work (approval channel + private thread); avoids Unknown interaction (10062).
        await interaction.deferUpdate();
        ...
        const restockReport = {
            ...
            source: 'button',
            region: region,
            line_estimate: lineEstimate
        };
        if (cachedData.newlook_alerts === true) {
            restockReport.newlook_alerts = true;
        }
        ...
        const approvalEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '🆔 Report ID', value: restockId, inline: true },
                { name: '📱 Source', value: 'Button Workflow', inline: true },
                { name: '👥 Initial Line', value: crowdTracker.formatEstimate(lineEstimate), inline: true }
            );
        ...
        let approvalSent = postedToApproval;
        ...
        try {
            ...
            console.error('❌ Error editing interaction after submit:', updateError);
            if (interaction.deferred) {
                try {
                    await interaction.editReply({
                        content: `✅ **Restock Report Submitted!**\n\n🏪 **Store**: ${store}\n\nYour report has been submitted${approvalSent ? ' and is awaiting moderator approval' : ' (approval channel may be misconfigured)'}.`,
                    ...
                    console.error('❌ Error on fallback editReply after submit:', replyError);
                }
            } else if (!interaction.replied) {
                try {
                    await interaction.reply({
...
/**
 * New Look: open modal for custom store (Other).
 */
async function prepareNlOtherStoreInProgressModal(interaction, region) {
    const sessionId = generateModalId();
    modalDataCache.set(sessionId, {
        userId: interaction.user.id,
        region,
    ...
    const modal = new ModalBuilder()
        .setCustomId(`nl_custom_inprog_${region}_${sessionId}`)
        .setTitle('Enter Custom Store Details');
...
/**
 * New Look: submit custom store modal → confirm step (same approve flow as legacy).
 */
async function handleNlCustomStoreInProgressSubmit(interaction, region) {
    try {
        const customId = interaction.customId;
        const sessionId = customId.replace(`nl_custom_inprog_${region}_`, '');
        ...
        cachedData.username = interaction.user.username;
        cachedData.reportType = 'in_progress';
        cachedData.newlook_alerts = true;
        modalDataCache.set(sessionId, cachedData);

        await interaction.reply(lineEstimatePromptPayload(cachedData, sessionId));
    } catch (error) {
        console.error(`❌ Error handling NL custom store (in-progress) (${region}):`, error);
        await interaction.reply({
            content: '❌ There was an error. Please try again.',
            ephemeral: true
        }).catch(() => {});
...
/**
 * New Look consolidated flow: stash session flagged for va/md new-look alert channels.
 */
async function stashNewlookInProgressConfirmation(interaction, region, store, storeType) {
    try {
        const userId = interaction.user.id;
        const username = interaction.user.username;
        ...
        console.error(`❌ Error in stashNewlookInProgressConfirmation (${region}):`, error);
        if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({
                content: '❌ There was an error. Please try again.',
                ephemeral: true
            ...
            try {
                ...
                await interaction.followUp({ content: '❌ There was an error. Please try again.', ephemeral: true }).catch(() => {});
            }
        }
    }
}
...
module.exports = {
    ...
    handleLineEstimateSelect,
    handleReportReviewNext,
    handleConfirmInProgress,
    handleConfirmPast,
    handleConfirmUpcoming,
    handleCancelReport,
    prepareNlOtherStoreInProgressModal,
    handleNlCustomStoreInProgressSubmit,
    stashNewlookInProgressConfirmation
};
